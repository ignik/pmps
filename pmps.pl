...
while(1) {
	...
	print "# multicast ".$S->peerhost.':'.$S->peerport.' to '.$S->sockhost.':'.$S->sockport."\n" if $V;
	...
	$T->send($NFY) or warn "Can't send NOTIFY to $p{ip} $!"; print "# NFY:\n$NFY\n" if $V > 1;
}
